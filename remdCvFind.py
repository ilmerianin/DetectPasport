...
def getFoto(path=pathFoto):
    ''' Генератор фоток из папки по умолчанию фотки должны быть в папке с относительным путем 'data/фото' 
    ...
    for file in files:
        ...
        if os.path.isfile(os.path.join(path, file)):
            ...
            image = cv.imread(os.path.join(path, file), -1)
                                    # -1: Загружает картинку в том виде, в котором она есть, включая альфу.
            if  image is None:
                continue
            else:
                print('фото:', file, ' shape:', image.shape)
                yield image, file

def saveImage(file, path=pathFoto,  paper = paperNew, finename= 'newfoto.jpg'):
    ...
    newPath = os.path.join(path, paper) 
    # проверка на наличие папки для новых файлов 
    if  not os.path.isdir(newPath):
        os.makedirs(newPath)
        print('папки нет создаю!')
        
    print(os.path.join(path, paper, finename))
    
    cv.imwrite( os.path.join(path, paper, finename), file)    
     
def viewImage(image,waiK= 500, nameWindow = 'Окошко Картинки'):
