...
paperNew = 'newYolovoRemd'           # папка где будут новые файлы, вложенна  в pathFoto т.е новые файлы будут в папке 'data/фото/newfoto'

verbose = False # если нуны всплывающие окна установить в True
remd= False   #True/False флаг применения Неиросети remd поиск фона работает долго и менее стабильно
...
def main():
    ...
    while nextFoto:              # цикл перебора фоток
        ...
        fileMod = 'tmp.jpg'                   # новое имя файла
            ...
            if 1 > abs(angel):  #  угол поворота меньше 1 гр. запустить обрезку и сохранить
                ...
                if remd:
                    ...
                    if True in imageRemdCut:
                        ...
                        #viewImage(imX,waiK =500 ,nameWindow='Remd Cut pred '+ str(len(line)- len(lineNow))+' '+str(len(lineNow))) # показать
                        flBadDetect = False # файл распознанн и сохраннен
                        angel = 0

                        print('\n  -------------------------------------------------------- \n rembg:', fileMod)
                        saveImage(imageRemdCut, path = os.path.join(pathFoto, paperNew) ,finename = file)
                        image= imX
                        break
        ...
        print('Итоговая:',fileMod)
        im0 , line  = predFoto(fileMod) # Проверка
